...
export function ArticleCard({ 
  ...
  return (
    /* 2. Bungkus dengan Link menuju /article/[id] dan tambahkan style hover/cursor */
    <Link 
      href={`/article/${article.id}`} 
      className="block group w-full cursor-pointer"
    >
      <article className="relative flex min-h-card-min-h gap-gap rounded-lg border border-card-border bg-background p-4 transition-all hover:border-primary">
        ...
        <div className="flex flex-1 flex-col gap-3.5 pr-card-pr max-[900px]:pr-0">
          <div className="flex flex-col gap-2">
            ...
            {/* 3. Tambahkan efek group-hover pada judul agar interaktif */}
            <h2 className="font-sans text-2xl font-bold leading-6.5 text-text-primary group-hover:text-primary transition-colors">
              {article.title}
            </h2>
