...
type LibrarySourceRow = { article_id: string };
...
export function LibraryContent({ initialTab }: { initialTab?: string }) {
  ...
  useEffect(() => {
    ...
    async function loadLibraryItems() {
      ...
      const activityColumn = source === "reading_history" ? "read_at" : "created_at";
      const { data: sourceRows, error: sourceError } = await supabase
        .from(source)
        .select(`article_id,${activityColumn}`)
        .eq(userColumn, userId)
        .order(activityColumn, { ascending: false });

      if (cancelled) return;
      if (sourceError) {
        console.error("Gagal memuat library:", sourceError.message);
        setLoadError("Gagal memuat library. Coba lagi.");
        setArticles([]);
      } else {
        const articleIds = [...new Set((sourceRows as LibrarySourceRow[] ?? []).map((row) => row.article_id))];
        ...
        const { data: articleRows, error: articleError } = await supabase
          .from("articles")
          .select("id,title,excerpt,cover_image,created_at,profiles!articles_author_id_fkey(username,display_name,avatar_url),likes(count),comments(count)")
          .in("id", articleIds);

        if (articleError) {
          console.error("Gagal memuat detail library:", articleError.message);
          setLoadError("Gagal memuat library. Coba lagi.");
          setArticles([]);
          setLoading(false);
          return;
        ...
        const mapped = (articleRows ?? []).flatMap((item) => {
          const article = item as unknown as LibraryArticle & { id: string };
  ...
  return (
    <section className="w-full">
      ...
      {loadError && <p className="mt-8 text-center text-sm text-danger">{loadError}</p>}
